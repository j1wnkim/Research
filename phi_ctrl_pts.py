...
def phi_ctrl_pts(
    ...
    if any(y[:,0] == None) or isinstance(y[:,0], (int, float, complex)):
...
def phi_extremes(y, xtrm_type, coef):
    ...
    bx_plt_st = box_plot_stats(y[:,0], coef)
    
    ## calculate range of the response variable y
    #rng = np.array([y.min(), y.max()])
    rng = np.array([y[0][0], y[len(y)-1][0]])
    #rng = [y.min(), y.max()]
    ...
    < bx_plt_st["stats"][0]):
        ...
        ctrl_pts = np.append(ctrl_pts, np.array([bx_plt_st["stats"][0], 1, 0]))
        
   
    ## min
    else:
        #ctrl_pts.extend([rng[0], 0, 0])
        ctrl_pts = np.append(ctrl_pts, np.array([rng[0], 0, 0]))
    
    ## median
    if bx_plt_st["stats"][2] != rng[0]:
        #ctrl_pts.extend([bx_plt_st["stats"][2], 0, 0])
        ctrl_pts = np.append(ctrl_pts,[bx_plt_st["stats"][2], 0, 0] )
    
    ## adjust high
    if xtrm_type in ["both", "high"] and any(bx_plt_st["xtrms"]
    > bx_plt_st["stats"][4]):
        #ctrl_pts.extend([bx_plt_st["stats"][4], 1, 0])
        ctrl_pts = np.append(ctrl_pts, np.array([bx_plt_st["stats"][4], 1, 0]))
    
    ## max
    else:
        if bx_plt_st["stats"][2] != rng[1]:
            #ctrl_pts.extend([rng[1], 0, 0])
            ctrl_pts = np.append(ctrl_pts, np.array([rng[1], 0, 0]))
