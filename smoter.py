...
def smoter(
    ...
    result_list = [(value, index) for index, value in enumerate(y)]
    ...
    ) # returns rarity values for each data point from a scale [0,1]
    ...
    ## ---------------------------------------------------------------------- ##
    if np.all(y_phi == 0):
        raise ValueError("redfine phi relevance function: all points are 1")
    
    if np.all(y_phi == 1):
        raise ValueError("redfine phi relevance function: all points are 0")
    ...
    for i in range(0, len(y_sort) - 1):
        if ((y_phi[i] >= rel_thres and y_phi[i + 1] < rel_thres) or 
            (y_phi[i] < rel_thres and y_phi[i + 1] >= rel_thres)):
                #bumps.append(i + 1)
                bumps = np.append(bumps, [i+1])
    
    #bumps.append(n)
    bumps = np.append(bumps, [n])
    ...
    if samp_method == "balance":
        for i in b_index:
            ...
            s_perc = np.append(s_perc, [b/len(b_index[i])])
            
    if samp_method == "extreme":
        for i in b_index:
            #scale.append(b ** 2 / len(b_index[i]))
            scale = np.append(scale, [b ** 2/len(b_index[i])])
        #scale = n_bumps * b / sum(scale)
        scale = n_bumps * b / np.sum(scale)
        
        for i in b_index:
            #obj.append(round(b ** 2 / len(b_index[i]) * scale, 2))
            #s_perc.append(round(obj[i] / len(b_index[i]), 1))
            obj = np.append(obj, [round( b ** 2 /len(b_index[i]) * scale, 2)])
            s_perc = np.append(round(obj[i] / len(b_index[i]),1))
    ...
    for i in range(n_bumps):
        ...
        if s_perc[i] == 1:
            ...
            data_new = pd.concat([data.iloc[b_index[i][:,1]], data_new])
