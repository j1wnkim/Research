...
def phi_init(y, n, num_pts, ctrl_pts):
    ...
    for i in range(num_pts):
        x = np.append(x, [ctrl_pts[3*i]])
        y_rel = np.append(y_rel, [ctrl_pts[3 * i + 1]])
        m = np.append(m, [ctrl_pts[3 * i + 2]])
    ...
    for i in range(num_pts - 1):
        ...
        delta = np.append(delta, [(y_rel[i + 1] - y_rel[i]) / h[i]])
        #h.append(x[i + 1] - x[i])
    ...
    for i in range(num_pts - 1):
        c = np.append(c, [(3 * delta[i] - 2 * m_adj[i] - m_adj[i + 1]) / h[i]])
        d = np.append(d, [(m_adj[i] - 2 * delta[i] + m_adj[i + 1]) / (h[i] * h[i])])
    ...
    for i in range(n):
        y_phi[i] = pchip_val(y[i,0], x, a, b, c, d, num_pts)
